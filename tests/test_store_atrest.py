...
def seed(owner):
    with owner.cursor() as cur:
        ...
        # ANN guardians both lanes: narrate_a_read serves as ANN, and with
        # migrations/003 applied a read is only served where the principal
        # holds a live edge. Before RLS this seed reached LCARA on nobody's
        # authority; now it must say whose.
        for lane in (LBEN, LCARA):
            cur.execute(
...
def write_both(app, custody):
    # ANN guardians both lanes (see seed): under migrations/003 an app-role
    # write only lands where the acting principal holds a live edge, so the
    # insert and its commit share one `acting` transaction.
    with acting(app, ANN):
...
def test_a_row_moved_into_another_lane_reads_as_misbound_not_as_that_lanes():
    ...
    with Database() as db:
        ...
        try:
            ...
            relabelled = sealed_rows(owner, LCARA)[0]
...
def test_the_app_role_cannot_insert_clear_bytes_into_the_sealed_column():
    ...
    with Database() as db:
        ...
        try:
            ...
            # owner, not app: under RLS the app role sees 0 rows whether or not
            # the attack landed, so this count would pass for the wrong reason.
            assert owner.execute("SELECT count(*) FROM lane_entry").fetchone()[0] == 0
        finally:
            app.rollback()
            app.close()
            owner.close()
...
def test_destroying_a_lanes_key_leaves_the_chain_verifying_over_the_live_store():
    ...
    with Database() as db:
        ...
        try:
            ...
            bens = sealed_rows(owner, LBEN)
            caras = sealed_rows(owner, LCARA)
    ...
    with Database() as db:
        ...
        try:
            ...
            assert atrest.unseal(old_rows[0], keyring=keyring,
                                 master=custody.master).plaintext == BENS_NOTE
            keys = {s.key_id for s in sealed_rows(owner, LBEN)}
