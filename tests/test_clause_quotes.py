...
* **Citations must be re-fetchable.** The tree cited Part III at `c8c96b4`,
  which resolves in no repository reachable from this session (§15's warning,
  made concrete). A dead object name may appear only on a line that says it is
  dead.
...
#: Object names this repository has anchored a Part III read on. `2886a41` is
#: the blob read on 2026-09-11; the other two locate it. Adding one means a new
#: read, and a new read means a new row in `docs/PART-III-READ.md`.
ANCHORS = frozenset({"2886a41", "7f2b3cf", "8c9c8e8"})
...
def test_every_part_iii_citation_is_refetchable():
    ...
    assert not bad, (
        "Part III cited at an object name this repository has not anchored, "
...
def test_w5s_unquoted_half_is_carried_by_the_read_record():
    ...
    assert W5_HALF in body, (
        "docs/PART-III-READ.md no longer names W-5's co-signature mechanism. "
