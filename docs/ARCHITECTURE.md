...
| §7.1 dated guardianship | `willow-2.0` `valid_at`/`invalid_at` | **VERIFIED 2026-07-30** at `4147013` — 17 tables counted from `migrations/20260522_bitemporal_all_tables.sql`, append-only audit excluded by name. **Mechanism exists**; binding guardianship to it does not. Note the source carries **no interval-ordering CHECK**; `migrations/001_lanes.sql` adds ten |
