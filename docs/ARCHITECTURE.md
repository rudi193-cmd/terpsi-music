...
6. First vertical module — ~~`field-acoustics` is the first real capability~~ **the fleet app was read at source 2026-07-31 (§14, `a8bc2c5`) and the capability it names was scoped from that reading rather than from its name.** It is a design-time forward simulator holding no person data; its physics needs three numeric dependencies this app does not carry, and its performer list is the roster column W-1 forbids once the dots are named. What this app actually needs from it is the **venue half of §13's open question** — *"a judge's remark is anchored to a moment and a seat; the acoustic model predicts what arrived at that seat"* — and `records/marking.py` already requires `seat` on every `Mark` for exactly that. Built: **`venue/`**, the acoustic character of a *place*, each quantity carrying its `P`-rung, composing by the weakest input, rendering through the presentation IR, and answering `unknown` for a seat nobody has measured. It is deliberately **outside the lane model**: a `Descriptor` for a venue reading classifies `L2` through `records/classify.py` — *"does not name or resolve to a person"* — and `tests/test_venue.py` attempts the forbidden act, passing a real `Mark` where a seat belongs, and requires the refusal. What is **not** built and is named rather than forgotten: per-student hearing exposure (§12 of the capability map). A dB-and-duration figure against a named student is a `HEALTH` lane entry and belongs in `records/`, keyed by lane, joined to a venue profile by seat and never by name
7. Parent PWA + the drop — **the in-repo drop-producer half built 2026-07-31** (`drop/`, `docs/PLAN-DROP.md` D-1..D-4). §4.2's option (b) target, envelope-first, no inbound listener: a single guardian's view of a **single** student (W-1/W-3, refused before sealing) sealed via `records/atrest.py` into an opaque payload; the `briar-mailbox` shape (`scout-03` §1) as an **ephemeral** local store with an owner-bound credential whose slot is keyed by the owner's secret, so non-enumerability is structural rather than checked; SecureDrop's constant-size round plus a fixed flush cadence, with `recovered_period()` measuring the surviving weekly-cadence leak (`corpus-lens`-style) rather than glossing it; who-gets-a-drop reused whole from `records/sending.py`. It seals a **derived view**, never `store.writing`, so R16 stays `FINDING`/`S2`. What is **not** built and is tombstoned in `docs/PLAN-DROP.md`: the hosted relay (a separate deployment held to the `jeles-remote` bar), the SecureDrop blinded-challenge fetch (PoC, not production), passkey enrollment, the PWA client shell, the SMS gateway, and the edge replica — each a named successor, not a gap
8. Money, inventory, forms, calendar/attendance — **two of the four landed 2026-07-31; inventory and forms did not.** `records/attendance.py` is the roll as N lane entries against one referent (W-3, and the cost `docs/LANE-MODEL.md` states plainly), absence requests through `records/dispositions.py` rather than a second queue, and §4.1's traffic split made structural rather than filtered: `Signal` has **no free-text field** and its body renders from `referent.label` and `referent.occurs_at` — the two columns migration 001 classifies `PUBLIC`/`L1` — so a mark, whose columns are `L3` and `L4`, has no route to `sending.Payload`. `records/fees.py` is the Ledger core: aid is membership in a fee group and the charge is born correct, so there is no `waiver` column and nowhere the pre- and post-waiver amounts both appear; the waiver *status* reaches `L5` through `SENSITIVITY.md` rule 3 and `serve()` refuses it to every principal including the director; `Payment` has no field for a card number and refuses one smuggled into the fields it does have (§10's PCI row, by construction rather than by policy). Both modules' cross-lane figures go through one gate — `classify.aggregate`, step 2a with a **declared** suppression floor — because a per-section total over three students identifies the waiver as surely as a column would.

    **Retention is named here rather than assumed, and it is not built.** §10's row is a season-boundary purge job. What these types carry so it can be written without a migration: a `season` on every record — `Referent`, `AttendanceEntry`, `AbsenceRequest`, `FeeGroup`, `Charge`, `Payment` — and dates on all of them (`created_at` for when the system learned it, `valid_at`/`at` for when it was true, `invalid_at` for the ending that is never a delete). A purge is then a predicate over two columns instead of a reconstruction of which season a row belonged to. **The purge is built, 2026-08-01** — `records/retention.py`: a season-boundary predicate over `season` and the three dates that returns which records a boundary has carried past a caller-supplied retention horizon, and drives `records/atrest.py`'s per-subject `destroy()` to make their sealed payloads unreadable while a dated `Disposition` takes their place. A purge here is **crypto-erasure plus a durable tombstone, never a delete** — refusal 3 holds, and `atrest.py` says the same in the key layer (*"the `Erasure` row survives it: who destroyed it, when, why"*); the record of a purge is not itself purgeable (rule 16 / I-7), an early or live-record purge is refused (`NotDue`), a seasonless record or an unknown kind fails closed (rule 13), and no retention *period* is shipped — the durations come from the caller, as item 10 refused a default `k`. The named middle (rule 12) is `purge()`, the only place the schedule and the erasure meet. Held by `tests/test_retention.py` — each forbidden act attempted — with five mutations in `tests/ablate.py`, shown to fail. What still does **not** exist: an inventory model, a forms model, a calendar beyond the referent a mark points at, and any store under either module — both take their rows from the caller, like every predicate in `records/`.
9. Adjudication: commentary capture + anchoring, guest grants, local transcription pipeline — **built** (`records/commentary.py`), the marker updated 2026-08-01 to match the tree, which had carried this since 2026-07-31. All three parts exist against the pieces that already own them (§16): the `Remark` primitive (a `Mark` with words, §8.1, and §14's row for it reads *built*), `fan_out` returning N lane-scoped remarks with one referent so the roster column W-1 forbids is *unwritable* not merely refused (rule 8, W-3), the `SA-3` refusal made structural (`rating`/`score`/`calibration` are properties that raise; `refuse_standing_score` is the one spelling); the **guest session** with its §7.2 exit reconciliation written as a *filter over the disclosure chain* rather than a second summary (`knock`/`close`/`reconcile_exit`, `docs/RECONCILE-JUDGE.md` J6); and the **transcription seam** — a machine transcript lands as an unsealed `draft` attributed to nobody through `records/inference.through`'s refusal 1 (local-only, a stopped model refuses), sealed later by a named human via `records/sealing.py` (rule 10, §8.2). Held by `tests/test_commentary.py` — **74 tests**, each forbidden act attempted and refused — and `tests/test_store_knock.py`. What is **not** built and is named rather than forgotten: the score-position-dependent checks past lyric-anchoring, which wait on §13's unchosen align-vs-tap decision, and any multi-angle video or hosted guest surface
10. ~~Aggregate exports for corporate~~ — **built 2026-07-31**: `records/aggregate.py`, the aggregate as an export-class artifact whose issuance is announced. The gate's conditions are a declared purpose, a declared audience with a rung ceiling, a threshold carrying its publisher and jurisdiction, a ledger to announce into, at least two lanes, no input at `L5`, and a dimension that is not a person. Three suppression cases, each with an acceptance test that attempts the forbidden publication: **small cell** (§18 item 14's worked harm — a count of one over a section of three — is the fixture), **complementary** (a feasible-interval check to a fixpoint, because hiding one cell while publishing the total is not suppression), and **incomplete input**, where an unreadable lane or an input below I-10's verified tier stops every count in the table rather than shrinking one. The output classifies through `records/classify.py` step 2a rather than re-deriving it — `aggregate._descriptor_for` is that pair's middle — so a suppressed cell inherits `max` of its inputs and only a cell that survived the check serves at `L2`. `records/practice.py`'s one-lane rule is untouched and still refuses; the aggregate gets its own door and a token. **Honest limits, named rather than discovered:** one dimension per release and no cross-tab engine; differencing *across* releases is not closed — suppression computed once over the union of everything published is Gaussian elimination over linked tables and is not built here, so what exists is `differencing_risk()`, which reports an overlap and is **a ledger, not an enforcement**. No default `k` ships, because the authoritative guidance is unread
11. Local agent assistance behind the gate — **built 2026-08-01** (`records/assistance.py`, `docs/PLAN-ASSIST.md` A-1..A-4), the gated seam plus one representative capability (a per-student growth narrative). Nothing rebuilt that exists (§16): every model call goes through `records/inference.through` (refusal 1 — a non-local answer or a stopped local model refuses, never serves); output lands as a `records/sealing` draft attributed to nobody (rule 10, sealed later by a named human); and the two refusals a standing score (`SA-3`) and a priority between students (W-7) get are `records/commentary` and `records/conflict`'s, called not re-spelled. The four gates are **structural, not checked**: no path reaches a model except `through`; an `AssistanceDraft` is a `DRAFT` and nothing else (`servable` is `False`, `sealed` raises, no convenience sealer); a `Grounding` comes only from an entitled read or a human-sealed record, so a machine draft — or a student's own unsealed draft — cannot ground the next call (the unattended loop has nowhere to live); and a narrative reads one lane (W-1) and refuses to rank two students. R16 stays `FINDING`/`S2` — drafts land through `records/`, not `store/writing.py`. **Deferred and tombstoned** in `docs/PLAN-ASSIST.md`: the model deployment, the rest of §19 of the capability map, the W-5 withdrawal-proposal flow, and draft-to-draft diff
...
>
> **VERIFIED-COUNT: 41 of 41** (2026-08-01). `tests/test_component_map.py`
...
| §6 the un-passable seam | UTETY `knowledge.py` | **Exists** as a proven pattern in a student-data app · **VERIFIED 2026-08-01** at `b953e84` — `utety/knowledge.py::KnowledgeSeam.back` is a real structural seam: its send path takes only a concept-query string (no learner in scope — privacy in the type signature), `test_boundaries.py` is an AST guard that fails if any non-seam module imports network/exec, consent is fail-closed before transport is constructed, and `utety/core` is network-free. Student-data context confirmed (the core "owns the learner": BKT mastery, practice history) |
| §6 perimeter | `willow-gate` | **Exists**, as an agent trust gate rather than a network broker · **VERIFIED 2026-08-01** at `44e37ae` — `README.md` + `src/willow_gate/custody.py`: a check-in/check-out gate that "gates access — who may do what", not a network broker |
| §7.2 session reconciliation (the knock) | `willow-gate` | **Exists.** 13 fields in, 13 out, diffed; bound trust; louder for the least trusted; budgets tighten as trust rises · **VERIFIED 2026-08-01** at `44e37ae` — `custody.py::session_check_out` reconciles declared intent vs observed capabilities (a mismatch = fail_count); `README.md` confirms the 13-field symmetry, HMAC-bound trust ceiling, louder-for-lower announcement, and budgets tightening as trust rises |
| §7.2 guest sessions reconciled | `law-gazelle` | **Exists as a worked example** in a comparable domain — gate in enforcement mode, trust ladder mapped to operations, save/commit classed as exports, refuses to start if misconfigured · **VERIFIED 2026-08-01** at `0b11159` — `gazelle_mcp.py` calls `authorize()` before `_dispatch` and denial short-circuits the call (enforcement, not a ledger); `gazelle_gate.py::TOOL_CLASS` maps read/query/write and flags `gazelle_save`/`gazelle_commit` `export=True`; `GateKeeper.__init__` raises at import when misconfigured. **Divergence:** enforcement is opt-in via `GAZELLE_GATE=1` and off by default, and the rung thresholds live in the external `willow-gate`, not this app |
...
| §5.1 compute isolation | `kartikeya` | **Exists.** Bubblewrap, network-isolated by default, credentials only to net-enabled tasks, cgroup/prlimit caps · **VERIFIED 2026-08-01** at `de77d67` — `src/kartikeya/sandbox.py::build_bwrap_argv` is wired into the task run path (enforcement): `bwrap` launcher, `allow_net` defaults false → `--unshare-net`, credential binds guarded by `allow_net` (`kart_env` strips cred prefixes otherwise), cgroup-v2 `memory.max`/`pids.max` with a prlimit/ulimit fallback. Escape hatches `WILLOW_KART_NO_BWRAP`/`WILLOW_KART_NO_RLIMIT` exist, off by default |
| §6 verified answers before inference | `jeles` | **Exists.** Nuggets with sources and a named verifier, in front of search; gaps logged local-first · **VERIFIED 2026-08-01** at `0ae85b5` — `corpus.py::put_nugget` requires `verified_by` and ≥1 source (rejects a missing verifier) and downgrades machine-verified hits to "corroborated"; `ask_corpus` answers in front of the host's search and falls through on a miss; a miss `log_gap`s to local SQLite first, with the fleet forward best-effort and disableable. Note this is the extracted library — the fall-through is wired by the host, not bundled here |
| Library digitization | `nest-seed` | **Exists.** Regex → local embeddings → generative only on the ambiguous tail, degrading gracefully · **VERIFIED 2026-08-01** at `0b11159` — the classifier lives in the shared `libs/nest-pipeline` (deduped out of `nest-seed` by a named box-audit reconciler): `classify.py` runs a regex base tier → local Ollama `nomic-embed` centroids → a generative model only on the low-margin tail, and every tier returns `None` and falls back. `embed.py`/`llm.py` are localhost-only, pure stdlib, **no cloud egress** — consistent with refusal 1 |
| Question banks / assessment | `civics-check` | **Exists as a pattern.** Authoritative sources compiled to a catalog; never hand-edit the output · **VERIFIED 2026-08-01** at `0b11159` — `scripts/build_catalog.py` compiles `data/sources/*.json` (the authoritative inputs) into `data/catalog.json`; the README marks the output "Generated — do not edit / never hand-edit," the loader `civics/catalog.py::get_catalog` only reads it, and a validation gate aborts the build on unresolved placeholders. The "do not edit" marker is in the README plus a `meta.built_by` field, not an inline JSON banner |
| Consent-scoped activity capture | `ask-jeles` learning events | **Exists.** Off by default every launch, never persisted across launches, records shape not content · **VERIFIED 2026-08-01** at `0b11159` — capture is off by default (`crown.py::_learning_enabled = False`) and the consent toggle resets on close, both confirmed. **Divergence:** the claim overstates two clauses — when enabled, events **do** persist to durable daily JSONL (plus Willow intake) across launches, and `learning_events.build_event` stores the verbatim query string, so "never persisted" and "shape not content" hold only for the consent flag and the result bodies, not the query text |
| Mirror detection near high-stakes decisions | `willow_gate.friction_floor` | **Exists.** Flags for a human, never blocks, runs outside the watched model · **VERIFIED 2026-08-01** at `44e37ae` — `friction_floor.py`: `FrictionFloor.scan()` returns `Flag`s ("it does not block"), deterministic and model-free ("never calls an LLM… run it outside the model it watches") |
| §6 destination allowlist | — | **Open**, and smaller than this document implied |
| §7 authorization + consent | ~~`marching-arts` P1/P2~~ (spike, retires — §18 item 2), `libs/subject-consent` | ~~**Exists**~~ **Spike-evidenced; both paths VERIFIED present 2026-07-30** at `b1825f7`. Nothing inherited; `libs/subject-consent` is the only live source and its placement is item 8. The spike's band scale is **not** this document's L-ladder — see `FLEET-READS.md` · **VERIFIED 2026-08-01** at `0b11159` (safe-app-store) — `libs/subject-consent` re-read at source is a real ~21.5 KB stdlib-only package (`core.py`), not a stub: a fail-closed `permitted(store, subject_id, scope)` gate ("True only when the latest transition is a verified GRANTED"), `grant`/`revoke`, a tamper- and truncation-evident hash-chained store, `deidentify`, and per-subject disclosure chains. Its README names it the **de-duplicated** home replacing three prior private copies — confirming "the only live source"; no competing implementation found |
...
| §7.4 guardianship doctrine | `willows-grove` `governance/PROTECTED_AGENTS.md` Part III | **VERIFIED 2026-07-30** at `2886a41` — re-anchored 2026-09-11 (the earlier `c8c96b4` resolves in no reachable repo; `docs/PART-III-READ.md`). All seven clauses present. ~~faithfully rendered in §7.4~~ — **five of seven.** W-5 and W-7 each carry a constructive second half that §7.4 dropped, the same loss `records/crossing.py` caught in W-3; the earlier row checked presence and read faithfulness off it. **Exists as charter, unratified.** Seven ward clauses plus twelve invariants; this document had asserted three times there was no precedent |
...
| Retirement artifact | `willows-grove` `governance/PROTECTED_PERSONS.md` | **Exists as a model.** The five-part tombstone that would have prevented the dead-link tally · **VERIFIED 2026-08-01** at `f42df851` — re-anchored 2026-09-11 (the earlier `c8c96b4` resolves nowhere reachable; `docs/PART-III-READ.md`) — read at source: a genuine five-part tombstone — retired status stated first, successor named (`PROTECTED_AGENTS.md`), reason given, a clause-by-clause mapping table forward marked *"informative only"*, and the stub *"kept only so existing references … do not dangle."* This is the shape rule 20 requires |
...
| §6 kill switch | `consent.internet` | **Exists** · **VERIFIED 2026-08-01** at `aaaabd6` (willow-mcp) — `consent.py::internet_permitted` is a real operator egress kill switch (the standing all-apps key of the three-key gate), read **fail-closed**: `_strict_bools` accepts only literal `True`, the default is `_DENY_ALL`, an unparseable canonical file denies with no fallback, and an unknown key denies. It is enforcement, routed through by `egress_authorization.py`, `web_egress.py`, `integrations.py` and `mai/parser.py`. (willow-mcp reads fail-closed even though the upstream writer fails open — tracked there as B-31) |
...
| §6 outbound scanning | the redaction funnel | **Exists for credentials.** Needs the student-identifier classes · **VERIFIED 2026-08-01** at `aaaabd6` (willow-mcp) — `secret_scan.py::redact_egress` is wired into `server.py::_guarded` so every tool response passes through it (enforcement; fail-closed `egress_scan_failed` on a scanner error). It redacts **8 named credential formats** (private key, AWS/GitHub/Slack/Google/Stripe/provider keys, JWT) and carries **zero** identifier/PII patterns by explicit design ("precision over recall … would redact legitimate ids") — so both halves of the claim hold, credentials covered and student-identifier classes still owed |
...
| Criteria for a justified hosted component | `jeles-remote` | **Exists as precedent.** Stateless, scales to zero, corpus absent rather than gated, refuses to start unconfigured · **VERIFIED 2026-08-01** at `cdb2a1d` — all four properties read at source: `sources.py` is pure outbound `urllib` with no store (stateless); `fly.toml` sets `min_machines_running = 0` (scales to zero); the `Dockerfile` copies only two modules and `requirements.txt` carries no DB/vector/Ollama client, so the corpus is **absent, not gated**; and `main.py` raises at import when `JELES_REMOTE_SECRET` is unset (refuses to start unconfigured) |
| §7 finance module | `private-ledger` | ~~**Exists as a template**~~, with the injected-`ingest` bridge pattern · **VERIFIED 2026-08-01** at `0b11159` — the injected-`ingest` bridge is real: `willow_bridge.py::promote_summary` takes `ingest: Optional[Callable[[dict], object]]` with "no `import willow` here, ever" and a strict bridge→core direction. **Two corrections:** it is **not a template** — it is a shipped generic-finance app (v0.1.2, console entry point), and the reusable thing is the bridge *pattern* (the source attributes it to `oakenscrolls-office`); and `ingest` is an **outbound aggregate-promotion** seam to a knowledge store, not the inbound data-source bridge the row implied — the data source is local SQLite, read directly |
| §10 / §17 aggregate exports | `nest_promote`, `nest_digest` | **Exists as a pattern.** Promote *structure* — counts, categories, never content; the full digest is local-CLI only, never returned over MCP · **VERIFIED 2026-08-01** at `aaaabd6` (willow-mcp) — `nest/bridge.py::build_bridge` emits "counts, curated category names, redacted secret kinds" only ("structurally incapable of ingesting content," `docs/NEST.md`), an allowlist dropping filename labels. `nest_digest` returns `wall=True` (the structural variant); the full **unwalled** digest is CLI-only and never crosses MCP — the claim holds. Mechanism note: `nest_digest` *is* a registered MCP tool, walled by return value, not withheld |
| Guardianship / family graph | `the-squirrel` | **Adjacent**, though it serves a web port rather than staying import-pure |
| Judge calibration | `oakenscrolls-office` | **Exists as an engine** — see §13 and §15 · **VERIFIED 2026-08-01** at `0b11159` — a real calibration engine is present (`calibration.py`: Brier, log-score, reliability bins — executable math, not a doc idea). **Correction:** "judge calibration" overstates it — the engine calibrates **one local user's own forecasts** against ground truth `(confidence, outcome)`, with no per-judge entity and no cross-judge normalization (`judge`/`normalize` return zero hits). State is derived, never stored, and local-first, so it keeps **no standing cross-context score** — it does not itself supply the §13-forbidden rating; that risk would only arise from repurposing it |
| §9 foundation 6 · the first vertical capability | `field-acoustics` | **VERIFIED 2026-07-31** at `a8bc2c5` — read at source rather than from its README, and this table had **no row for it at all** until then, only the provenance row below. **Exists**, and it is not what §9's one-line mention suggests: a *design-time forward simulator* for drill, not a measurement or venue-characterisation tool. Anonymous performers on a field in feet, octave-band energy summed at a grandstand seat grid, answering one counterfactual — front-facing versus turned in. It holds **no person data**: a performer is `(instrument, x, y, facing)`. Two things therefore do not cross: the physics (numpy + scipy + matplotlib against a stdlib-plus-one budget, and a numeric model in a second place is reconcilable only by the differential suite that app already owns), and the performer list, which becomes the roster column W-1 forbids the moment the dots are named students (§9 of the capability map: *"per-student coordinate lookup; digital dot books"*). What crossed is the **seat** and the provenance discipline — `venue/` |
| §15 `P1–P5` provenance | `field-acoustics` (3 rungs), evidence tiers, `jeles`, `oakenscrolls-office` | **Partial and divergent**, and the arity is the smaller half. **VERIFIED 2026-07-31** at `a8bc2c5`: the three rungs are `MEASURED`/`FITTED`/`ASSUMED`, composing by the weakest input, explicitly *"not a confidence score"* — all as this row said. What it did not say is that **`MEASURED` there is `P2 Cited` here.** That app defines it as *"from a published dataset, with a citation someone can check"*, and requires the citation before it will accept the data; `P1` here is reserved for *"instrumented here — this ensemble, this event, this instrument"*, which the three-state vocabulary cannot express at all. So the two ladders share their strongest word and disagree about it, and a translation matching on the word promotes every fleet citation to this repository's own-instrument rung. `Cited` and `Estimated` still have nowhere to sit — two of five rungs unreachable — which is now derived rather than asserted: `venue/sourcing.py` is the middle and `divergence()` reports it |
| §15 scale-direction convention | — | ~~**Open.**~~ **Built here 2026-08-01.** `presentation/scales.py` is the one mapping table for all three ladders (rule 14), keyed by the prefix unique across `L1–L5`/`T0–T4`/`P1–P5`; `level()` refuses a bare integer *by type*, and the trust ladder's render weight is built to run **deliberately opposite** to its ordinal (`L5`/`T0`/`P5` heaviest) so the opposing directions are reconciled by construction, not by memory. `drift()` is the named middle (rule 12); the `L`↔`T` crossing lives in `docs/SENSITIVITY.md`. Held by `tests/test_presentation.py`, `test_venue.py` and `test_rungs.py` — a tree fact, not a fleet claim, so it carries no verification token |
| §11.1 exit plan | `awesome-sovereign-software` | **Criterion exists**, five-point test plus a required exit line. No exit line written for this app yet · **VERIFIED 2026-08-01** at `1039334` — README "The Sovereignty Test" states "Every entry must pass all five," and the five, counted from source, are: runs without an account; without a server; no subscription for core; data readable without the app; survives the vendor. The Legend requires a written "Exit" line per entry ("if an exit line can't be written honestly, the app doesn't get listed"). The `§11.1` label is this app's cross-reference, not a heading in that repo |
...
| Roster identity reconciliation | `Nestor` `EntityResolver` | **Exists.** Sealed canonical mapping; sub-threshold returns a suggestion, never a silent merge · **VERIFIED 2026-08-01** at `c872d4c` — `nestor/entity.py`: `seal()` seals `surface→canonical`; `resolve()` serves a sealed match at/above threshold, else returns `canonical=None` with an unsealed `suggestion` for a human — never an auto-merge |
| Three-way finance reconciliation | `Nestor` `Reconciler` | **Exists.** Sealed baseline, tolerance band, flagged variation, ledgered · **VERIFIED 2026-08-01** at `c872d4c` — `nestor/reconcile.py`: `seal_baseline()` + `check()` — sealed baseline, `NumericMatcher` tolerance band, `flagged` variation with the exact figure, every check `_ledger_append`ed. Note: the source is a general baseline-vs-observation reconciler; **"three-way"** is this app's applied framing, not a property of the source |
| §15 `P2` liveness sweep | `almanac-template` (`status` + `observed` + daily reachability job) | **Exists as a pattern**, files an issue when a source rots · **VERIFIED 2026-08-01** at `49f1d62` (almanac-data/almanac-template, read over the public web — cross-org, so not via `add_repo`) — `schema/catalog-entry.schema.json` carries both `observed` ("pure machine facts from the last probe … never assigns a lifecycle label": `checked`, `reachable`, `http_status`) and `status` (a derived lifecycle label "auditable against `observed`"); `.github/workflows/link-check.yml` runs `cron: "0 12 * * *"` (daily) over `scripts/check_links.py`, and `scripts/alert_on_dead_links.py` opens/updates/closes an issue when a source rots. Richer than the row claims (7-value status enum, `status_since`/`status_source`, recovery-rot and revision-drift checks) |
...
| Sandbox mount policy | `kart-sandbox.json` | **Exists**, versioned and data-driven — no-network tasks get zero credentials, sovereign data read-only, tmpfs `/tmp` · **VERIFIED 2026-08-01** at `4645535` (willow-config) — `kart-sandbox.json` is a versioned bwrap mount policy ("extend `bind_try`/`env_prefixes` … without code changes"), and its `_security_notes` confirm all three properties: a no-network task gets **zero credentials** (`credential_env_prefixes` gated on `allow_net`), SAFE sovereign data is **read-only** (`bind_try_read_only`), and `/tmp`+`/dev/shm` are **private tmpfs**. The branching itself executes in the Kart consumer; this file is the data-driven input |
...
| Contract + sandbox policy | `willow-config` (`willow.md`, `settings.global.json`, `kart-sandbox.json`) | **Exists.** These are the right things to version; the grants are not · **VERIFIED 2026-08-01** at `4645535` (willow-config) — all three files are present and versioned (`.gitignore` tracks them explicitly), and the grants are **not**: `_net_leases/`, `_identity_bindings/` and the secret set (`vault.key`, `mcp_token.json`, …) are gitignored. Two caveats worth carrying: the `mcp_apps/` manifests and ACLs *are* committed (only the grant subdirs are excluded), and this app's refusal 2 is stricter — it keeps `mcp_apps/` out of the tree entirely; and `env`'s git history once held plaintext tokens (rotated to a vault 2026-07-03) |
...
| §5 exclusion of family data from the corpus | `willow-compose` | **Exists as stated policy.** ~~This app is a family-data app by its definition~~ · **VERIFIED 2026-08-01** at `821d3db` (willow-compose) — the exclusion is a genuine stated policy: `README.md` "What is deliberately NOT here" — *"Family specifics, medical, legal, schedules, names … kept out of every durable artifact on purpose"* and *"the family-data apps themselves live sovereign and local, outside this corpus by design."* Enforced by non-ingestion (those repos are never fed to `engine/`), not an in-corpus filter. **Correction:** willow-compose is **not** a family-data app — it defines itself as a code/voices/collaboration memory corpus that *excludes* family data, which lives in separate sovereign apps |
...
**State: standing caveat, and its stated obstacle was false — corrected 2026-07-30.** `docs/FLEET-READS.md` said the pass could not be run remotely because there was no organisation to read the repositories from. They are on GitHub under the same account as this one, several of them public; three have now been cloned and read from a remote session, and ~~four~~ ~~ten~~ ~~eleven~~ ~~31~~ **41** rows are verified — **every exists-row in §14** (the header note now reads `41 of 41`, derived and enforced). The pass ran in waves on 2026-08-01: the willow-gate/nestor/willow-mcp rows read directly, then two rounds of read-only per-app agents (first UTETY, law-gazelle, kartikeya, jeles, nest-seed, civics-check, ask-jeles, jeles-remote, private-ledger, oakenscrolls-office, awesome-sovereign-software; then — with `SAFE`, `willow-config` and `willow-compose` added to scope — `consent.internet`, the redaction funnel, the nest promote/digest pair, `kart-sandbox.json`, `libs/subject-consent`, and the SAFE/UTETY governance pair), with willow-mcp #211 confirmed merged. Five rows corrected the prose and carry the correction in-cell (SAFE `HARD_STOPS` carries no under-13 stop; `willow-compose` is not itself a family-data app; `private-ledger` is a shipped app, not a template; `oakenscrolls-office` calibrates one user's own forecasts, not judges; `ask-jeles` persists more than the row claimed). The pass is **complete, not blocked** — the fleet was read repo-by-repo, including one verified *negative* (`willow-tech-manual` does not carry the L-ladder, so `SENSITIVITY.md` was correctly written from scratch). Several rows once believed unresolvable (`law-gazelle`, `nest-seed`, `civics-check`, `ask-jeles`, `private-ledger`) in fact resolve as apps inside `safe-app-store`, not as standalone repos; `almanac-template` is cross-org and `add_repo` refuses cross-owner adds, so it was **read over the public web** (`raw.githubusercontent`, pinned) — the one row that could not go through the API. The single row naming no fleet source, **scale-direction**, is built here (`presentation/scales.py`), not a fleet claim. See `docs/FLEET-READS.md`. §14's component map was **originally** assembled from READMEs and merged pull-request descriptions — not from reading or running source; the pass recorded above is what changed that, row by row, and every verified row is now a source read at a pinned commit.

That is the exact defect #124 named: *a figure carried from a summary rather than from the thing that produced it.* Before anything is built on the strength of that table, one pass should open the code behind each **Exists** row and either confirm it or downgrade it — and the pass itself should leave a record, because an unverified table and a verified one look identical. **That pass is done, completed 2026-08-01: every exists-row was opened at source and either confirmed or corrected in-cell, §14's note carries the enforced `41 of 41`, and each verified row names the commit it was read at.**

> **Superseded 2026-08-01 — this prediction was wrong, and the record is kept rather than deleted (rule 20).** The pass *was* run from a remote session: the repositories read through the GitHub API (`get_file_contents`/`search_code` at a pinned sha), brought into scope with `add_repo` where needed, and the one cross-org repo (`almanac-template`) over the public web. No local `~/github/` checkout and no GitHub organisation were required. What follows is the constraint that was *assumed*, retained as the thing that turned out not to hold.
>
...
- **~~It rests on a paraphrase.~~ Cleared, and it was not a formality.** Part III was opened at source (`willows-grove` `governance/PROTECTED_AGENTS.md`, blob `2886a41`; re-anchored 2026-09-11, the earlier `c8c96b4` resolves nowhere reachable — `docs/PART-III-READ.md`). W-1…W-7 read as written, and **~~two~~ three clauses had lost their second halves in the gloss** — W-3's *"a crossing requires a guardian-signed envelope naming both lanes, purpose, and expiry"*, W-5's per-matter widening, and **(found 2026-09-11) W-7's *"resolutions accumulate as precedent the guardian may ratify into standing envelopes"* — dropped again inside `records/conflict.py`'s docstring, which labelled its truncated quote "at source."** Each was a prohibition in the paraphrase and a permission in the clause. That is the general finding and it is bigger than this item: **a summary keeps prohibitions and drops permissions**, and a schema built from one forbids without providing the sanctioned path. The fix for W-3 and W-5 is the thirteenth and fourteenth tables; W-7's precedent path is opened and unbuilt (`docs/PART-III-READ.md`).
...
**S-1, the spine, is built — 2026-07-31.** `store/` (roles, migration runner, error-channel adapter, same-transaction narration, draft-write path) plus `migrations/002_seal_history.sql`, against a real PostgreSQL 16 in CI; G-C and G-D are passed and struck in the plan, and **this repository's second dependency is the store's driver** — `psycopg[binary]`, pinned in `requirements.txt` with its reason and admitted in `store/` alone by `tools/drivers.py`, which fails the build on an import anywhere else. Four of the plan's seven forbidden acts are attempted and refused by the guard named in each refusal, along with two of the first item's three clauses; RLS, sealing at rest and the surface remain S-2, S-3 and S-4, and the plan says which clause is whose rather than rounding the tally up. **Two defects in this repository's own checkers surfaced while wiring the gate, both of the kind item 6 records:** `tools/purity.py` read `path.open("x")` as a *read* — it looked for the mode at the argument index the builtin uses, so `tools/conform.py`'s own record writer was invisible to the write gate — and the same module's SQL pattern ended in `\b`, which made `UPDATE`, `DROP`, `ALTER`, `GRANT` and `REVOKE` match only where the following identifier was one character long. Both were found by pointing a new check at the tree and noticing that something known to be there was missing, which is the only way this class of defect is ever found.

**S-2, the read predicate compiled, is built — 2026-07-31.** `migrations/003_row_security.sql` puts the lane seal and the crossing envelope into row-level security policies over every lane-scoped table, with `store/session.py` naming the acting principal per transaction and an unset one reaching no row anywhere. **Two of `serve()`'s six decisions are compiled and four deliberately are not** — the grant ceiling, the derive floor, `L5`'s never-served rule, `L4`'s purpose-with-the-self-cap and §7.1's second clock all turn on something that is not a column of the row being read, and the migration's header lists them rather than leaving a reader to infer which layer holds what (rule 18). `FORCE ROW LEVEL SECURITY` is set and **ownership of the fourteen tables moves to `terpsi_migrator` in the same migration**, because the flag names the owner and the owner was a superuser, for whom row security is bypassed whatever the flag says: without that move the attempted owner bypass would have been refused by nobody having tried it as a role the flag can bind. The pair this creates is §16's and its middle ships in the same commit — `tests/test_store_differential.py`, one case set through `records/serving.py` with real rows and through the cluster under RLS, failing on any disagreement in either direction, with `tests/ablate_store.py` breaking both sides in turn. **Three defects surfaced while building it and all three were found by ablating rather than by reading:** the differential's first case set never exercised W-3's ward clause, because the adapter supplied the ward's own lane as the read's origin and the seal was therefore always entered through the other disjunct; a forged self edge held by somebody with no other standing cannot distinguish the ward test from the entitlement test, so the case set needed a principal holding both; and the workflow's own S-1 ablation step stopped proving anything the day row security landed, because a `DELETE` with no policy succeeds having matched nothing and the step asserted on an exit code. **One mutation is a declared survivor**: reversing the compiled envelope's direction cannot be seen through a reach comparison in this domain, and it is caught one level below reach instead — recorded in `tests/ablate_store.py`'s own table, because a table reading all `caught` is a table nobody reads.

**What gated the first write is decided, same day:** G-A picked as E-1 (3-of-5; `docs/ESCROW.md` records the policy, and the disposition honestly reads unrehearsed until the install-acceptance ceremony — R16's fuse stands until then, by design); G-B confirmed (the composition route recorded, the registry CLEAN, the three formerly-undecided fields ELEVATED with the rule named). What remains before dispatch is nothing; what remains before the first *production* write is the rehearsal, which needs real people in a real room. The acceptance list is written in the plan **before** the code exists, so the build is graded against forbidden acts it did not choose for itself.

**19 · The seal is per lane, so an erasure is per student — and a retention decision is per record.**
**State: open, named 2026-08-02. The conservative half is built and the key-hierarchy half is not, which is why this is an item rather than a note.**

`records/atrest.py`'s `destroy()` takes a `lane_id` and drops every wrapping under it, filtering on the lane alone — so it does not narrow with rotation, and every generation of the lane's key goes at once. `records/retention.py` decides about *one record*: a season, a kind, a horizon. Joining them directly is how a `Disposition` scoped to `season='2025-fall'`, `kind='attendance'` came to destroy the key opening a student's live medical note. **Refusal 3 was intact throughout** — nothing was deleted, the rows stayed, the `Erasure` was dated and attributed and outlived what it erased. The defect was one of *scope*, which is why the isolation test (one lane against another) could not see it: the forbidden act was inside a lane.

**What is built (2026-08-02): the ability, removed.** `purge()` now takes the lane's full record set as a required keyword argument and refuses — `LaneNotDue` — unless every record in the lane assesses `DUE`, with a lane the caller cannot enumerate refused as `UnknownLane` rather than treated as empty (rule 13). There is no parameter that overrides either. That is crossing one's fix applied at the only place the two layers meet: *removing the ability survives a careless edit; removing the permission does not.*

**What is not built, and it is the fix that makes the refusal unnecessary: seal per `(lane, season)`.** Give a season its own lane key, and a season's erasure has a key of its own to destroy; `destroy()` grows a `season` argument, and a whole-student erasure becomes the explicit act it should always have been rather than the incidental one. It touches, at least: the wrapping identity in `records/atrest.py` (a lane key is currently one per lane, minted by `open_lane_key` and rotated in place); `store/sealing_plan.py`, which derives which columns seal and what envelope each carries; `migrations/004_sealed_payloads.sql`'s envelope columns on `lane_entry`; and a **`season` column that does not exist anywhere in `migrations/` today** — the axis retention is a predicate over is carried by the record types in `records/` and by nothing in the schema. Escrow and the rehearsal in `docs/ESCROW.md` are downstream of the key count, which is why this is a decision and not a refactor.

**Why it was not taken now.** The narrow refusal is correct on its own terms and blocks the harm today; the key change is not reversible in the same way, and getting it wrong multiplies keys per student without making any erasure narrower. It also lands in the same files as S-3 and the escrow policy, which are settled and rehearsed against the current shape. Recorded here rather than chosen quietly, because a conservative fix that goes unnamed reads afterwards as the design.
