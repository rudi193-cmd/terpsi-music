...
>
> **VERIFIED-COUNT: 41 of 42** (2026-08-01). `tests/test_component_map.py`
...
**State: standing caveat, and its stated obstacle was false — corrected 2026-07-30.** `docs/FLEET-READS.md` said the pass could not be run remotely because there was no organisation to read the repositories from. They are on GitHub under the same account as this one, several of them public; three have now been cloned and read from a remote session, and ~~four~~ ~~ten~~ ~~eleven~~ **31** rows are verified (the *four* was true when written and drifted; the *ten* was true until the `apps/jarvis` row was added 2026-07-31; the *eleven* held through 2026-07-31; a 2026-08-01 pass read source for the willow-gate/nestor/willow-mcp rows and then, via read-only per-app agents, eleven more — UTETY, law-gazelle, kartikeya, jeles, nest-seed, civics-check, ask-jeles, jeles-remote, private-ledger, oakenscrolls-office, awesome-sovereign-software — three of which diverged from the prose and now carry the correction in-cell; the figure is derived and enforced, see §14's note). The pass is open, not blocked — **the fleet was read repo-by-repo as of 2026-08-01, with every exists-row now marked `VERIFIED` or `UNVERIFIED`**, including one verified *negative* (`willow-tech-manual` does not carry the L-ladder, so `SENSITIVITY.md` was correctly written from scratch). Several rows once believed unresolvable (`law-gazelle`, `nest-seed`, `civics-check`, `ask-jeles`, `private-ledger`) in fact resolve as apps inside `safe-app-store`, not as standalone repos; `almanac-template` remains unreachable cross-tier (a different org, and `add_repo` refuses cross-owner adds). See `docs/FLEET-READS.md`. §14's component map was assembled from READMEs and merged pull-request descriptions — **not from reading or running source.** Every row asserting that something exists is a `P2 Cited` claim (§15) whose source was read and never executed.
