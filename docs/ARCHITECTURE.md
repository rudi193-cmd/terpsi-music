...
- **~~Three~~ ~~Four invariants are stated and unenforced~~** — **State: closed 2026-07-31.** ~~I-7's supersession asymmetry, W-3's default deny, the rung ceiling, and (added 2026-07-30) **the `self` edge's holder**, which must be the lane's subject and cannot be expressed as a CHECK. The first three are predicates over the acting principal and belong with the read predicate; the fourth is a row-shape constraint needing a trigger, and it is the first entry added by *widening* the schema rather than by reading it. All four named in `LANE-MODEL.md` rather than left to be discovered.~~

  The `self` edge's holder closed 2026-07-30 with `records/standing.py`'s `is_self_edge()`. The other three closed together 2026-07-31, and the reason they closed together is the reason they were listed together: **all three were predicates over the acting principal**, so all three landed in `records/` beside the read predicate rather than as ledgers next to it. `LANE-MODEL.md` now names the path for each — `may_supersede()` called from `sealing.reject()`/`redraft()` for I-7, `serve()`'s `_acting_ward()` for W-3's default deny, `serve()`'s `_ceiling()` and the `Grant` type for the rung ceiling — with each guard's ablation label, and with the ceiling's residual stated rather than implied.

  **Two defects were found by enforcing them rather than by reading them, and both were live.** A ward was served another ward's `L2` lane entry with nothing consulted, because a read passing no `lane_id` never reached the seal and a field below the derive floor needs no edge. And `crossing.permits()` took `subject_id` with a `None` default that skipped the signer's standing check entirely, so an envelope signed by an ex-guardian still opened the seal for any caller who did not name the ward — invisible from the read path, which always named it.

  The fifth entry on `LANE-MODEL.md`'s list, **the hash chain, is untouched and open.** It is not a predicate over a principal, so it did not travel with these three.
