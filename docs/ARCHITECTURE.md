...
| §7.4 guardianship doctrine | `willows-grove` `governance/PROTECTED_AGENTS.md` Part III | **VERIFIED 2026-09-11 at `2886a41`** — re-read at source and partly corrected; first read 2026-07-30 (`docs/PART-III-READ.md`). All seven clauses present. ~~Faithfully rendered in §7.4~~ — **five of seven.** W-5 and W-7 each carry a constructive second half that §7.4 drops, the same loss `records/crossing.py` found in W-3; the 2026-07-30 row checked presence and read faithfulness off it. The earlier citation `c8c96b4` resolves in no repository reachable from this session. **Exists as charter, unratified.** Seven ward clauses plus twelve invariants; this document had asserted three times there was no precedent |
...
- **~~It rests on a paraphrase.~~** **State: closed 2026-09-11 — `docs/PART-III-READ.md`.** Part III was read at source in `willows-grove` (blob `2886a41`), and this bullet was **already false when written**: §14's own component map and `docs/FLEET-READS.md` had marked it VERIFIED on 2026-07-30, the same day. One document saying both for six weeks. The translation risk this bullet named was **real and had already occurred** — W-3, W-5 and W-7 each carry a constructive second half, and all three were dropped in compression; `records/crossing.py` caught W-3's and predicted W-5's, and W-7's had been dropped again inside a docstring labelled *at source*. W-1, W-2 and W-6 came through whole, so the structural encodings stand. The read opened four new items (W-5's co-signature, W-7's precedent, `access_grant.signer_id`, `edge` under I-3) — **it removed a gate and lengthened the list**, which is what a real read does.
