...
| Cloud inference fallback | `willow-seed` (Groq/Cerebras/SambaNova) | **VERIFIED 2026-07-30** at `a9274e8` — chain verbatim in `README.md:148` / `QUICKSTART.md:181`. **Must be disabled, not unused**, and harder than that: `willow-seed` has *no code* for it, the implementation is spread across ≥6 files in `willow-2.0`, no off-switch was found, and the only dispatch located tries cloud first with no local step |
