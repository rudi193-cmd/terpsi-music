...
UNDECIDABLE: Tuple[Callable[[], Check], ...] = (
    ...
    _unknown("named-middles", "a named middle for every pair the app creates (§16)",
             "not mechanically decidable. Six middles are named and tested "
             "(crossing, standing.is_self_edge, marking.drift, practice._one_lane, "
             "atrest.reconcile, atrest.composes); whether that is *every* pair is "
             "a reading, not a check"),
)
...
CHECKS: Tuple[Callable[[], Check], ...] = (
    ...
    check_component_map, check_declared_sockets, check_key_escrow,
) + UNDECIDABLE
