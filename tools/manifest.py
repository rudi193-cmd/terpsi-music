...
def _all_python() -> Tuple[Path, ...]:
    return tuple(p for p in sorted(ROOT.rglob("*.py"))
...
def sources() -> Tuple[Path, ...]:
    ...
    to add it to a list."""
    return tuple(p for p in _all_python() if _excused(p.relative_to(ROOT)) is None)


def excused() -> Tuple[Path, ...]:
    """The complement of `sources()` — enumerable, so it can be reviewed."""
    return tuple(p for p in _all_python() if _excused(p.relative_to(ROOT)) is not None)
